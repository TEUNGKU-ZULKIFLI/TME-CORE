...
def block_ip(api, ip_address):
    ...
    try:
        ...
        existing_lists = address_list.get(address=ip_address, list=config.ADDRESS_LIST_NAME)
        ...
        address_list.add(
            list=config.ADDRESS_LIST_NAME,
            address=ip_address,
            timeout=config.BLOCK_TIMEOUT,
            comment="Auto-blocked by TME-CORE"
        )
        print(f"[+] SUKSES: IP {ip_address} berhasil dimasukkan ke Address List '{config.ADDRESS_LIST_NAME}'")
        return True
